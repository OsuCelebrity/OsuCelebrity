...
public class OsuApplication implements Runnable
{
	...
	public void outputInformation(OsuApiUser currentUser, OsuApiUser nextUser)
	{
		...
		try
		{
			...
			saveImage("https://s.ppy.sh/images/flags/"+currentUser.getCountry().toLowerCase()+".gif", outputPath + "flag.gif");
		}
	...
	private String getWindowTitle()
	{
		String line = "";
		try {
			Process p = Runtime.getRuntime().exec("cmd /c for /f \"tokens=10 delims=,\" %F in ('tasklist /nh /fi \"imagename eq osu!.exe\" /v /fo csv') do @echo %~F");
